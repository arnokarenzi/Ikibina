...
import { Coins, Clock, Play } from "lucide-react";
...
export default function Contributions() {
  ...
  const handleRecordPayment = async (e) => {
    e.preventDefault();
    try {
      await api.post("/contributions/pay", {
  ...
  const handleExecuteCutoff = async (e) => {
    ...
    try {
      const res = await api.post("/contributions/cutoff", {
        targetDate: cutoffDate,
      });
      setCutoffResult(res.data);
    } catch (err) {
      console.error("Failed to execute cutoff:", err);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage your cooperative daily contributions.
        </p>
      </div>

      <div
        className={`grid grid-cols-1 ${isCommitteeMember ? "lg:grid-cols-2" : ""} gap-8`}
      >
        {/* Record Personal Payment */}
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
            <div className="p-2.5 bg-blue-50 text-coop-primary rounded-lg">
              <Coins className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-bold text-slate-800">Record Daily Payment</h2>
              <p className="text-xs text-slate-500">
                Standard 5,100 RWF deposit
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleRecordPayment} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-coop-primary hover:bg-blue-900 text-white font-semibold rounded-lg text-sm"
            >
              Record Contribution
            </button>
          </form>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            ...
            <form onSubmit={handleExecuteCutoff} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-purple-700 hover:bg-purple-800 text-white font-semibold rounded-lg text-sm flex justify-center items-center gap-2"
              >
                <Play className="w-4 h-4 fill-current" />{" "}
                {cutoffLoading ? "Executing..." : "Run Engine"}
              </button>
            </form>
            ...
            {cutoffResult && (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg text-xs space-y-1 text-emerald-800 font-mono">
                <div>
                  Disbursed: {cutoffResult.payout_amount?.toLocaleString()} RWF
                </div>
              </div>
            )}
          </div>
