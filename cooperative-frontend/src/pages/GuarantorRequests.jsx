...
export default function GuarantorRequests() {
  ...
  const handleAction = async (loanId, action) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || 'Failed to process action.');
    } finally {
  ...
  const handleDisburse = async (loanId) => {
    ...
    try {
      ...
      setMessage(res.data.message || 'Loan disbursed successfully and recorded in Ledger Audit.');
      await fetchGuarantorRequests();
    } catch (err) {
      setError(err.response?.data?.error || err.message || 'Failed to process loan disbursement.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Guarantor Requests & Disbursements</h1>
        <p className="text-sm text-slate-500 mt-1">Review emergency guarantee requests and manage fund disbursements for approved loans.</p>
      </div>

      {message && <div className="p-3 bg-emerald-50 text-emerald-800 rounded-lg text-sm flex items-center gap-2"><CheckCircle2 className="w-4 h-4" />{message}</div>}
      ...
      {loading ? (
        <div className="text-sm text-slate-500 p-8 text-center">Loading requests...</div>
      ) : requests.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-xl border border-slate-200 shadow-sm space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">You currently have no pending guarantor requests or active disbursement actions requiring your sign-off.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {requests.map((req) => {
            ...
            const isReadyForDisbursement = req.status === 'APPROVED' || req.fully_guaranteed === true;

            return (
              <div key={req.loan_id} className="p-5 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-slate-900">Loan #{req.loan_id}</span>
                    <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${isReadyForDisbursement ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                      {isReadyForDisbursement ? 'Ready for Disbursement' : 'Pending Sign-off'}
                    </span>
                  </div>
                  <p className="text-sm text-slate-700">
                    Borrower: <strong className="text-slate-900">{req.borrower_name}</strong> ({req.borrower_phone || 'No phone'})
                  </p>
                  <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-500 pt-1">
                    <div>Principal: <strong className="text-slate-800">{Number(req.principal_amount).toLocaleString()} RWF</strong></div>
                    <div>Your Guarantee Share: <strong className="text-slate-800">{Number(req.guaranteed_amount || 0).toLocaleString()} RWF</strong></div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end md:self-center">
                  {!isReadyForDisbursement ? (
                    <>
                      ...
                      >
                        <Check className="w-4 h-4" /> {isLoading ? 'Processing...' : 'Approve Sign-off'}
                      </button>
                      <button
                    ...
                    isCommitteeMember && (
                      ...
                      >
                        <Send className="w-3.5 h-3.5" /> {isDisbursing ? 'Disbursing...' : 'Release Money & Post to Ledger'}
                      </button>
                    )
                  )}
                </div>
