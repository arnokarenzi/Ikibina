...
export default function Loans() {
  ...
  // Application State
  const [selectedMember] = useState(user?.id || "");
  ...
  const fetchLoans = useCallback(async () => {
    ...
    try {
      ...
      const filteredLoans = isCommitteeMember
        ...
        : res.data.filter(
            (l) =>
              String(l.borrower_id) === String(user?.id) ||
              String(l.member_id) === String(user?.id),
          );
  ...
  }, [fetchMembers, fetchLoans, fetchRepayments]);
  ...
  const handleApplyLoan = async (e) => {
    ...
    try {
      ...
      if (loanTier === "TIER_1") {
        ...
        preparedGuarantors = [
          parseInt(guarantor1, 10),
          parseInt(guarantor2, 10),
        ];
      }

      const payload = {
        member_id: parseInt(selectedMember || user?.id, 10),
        tier: loanTier,
      ...
      setApplyMessage(
        res.data.message || "Loan application submitted successfully.",
      );
      fetchLoans();
    } catch (err) {
      setApplyError(
  ...
  const handleCommitteeApproval = async (loanId, action) => {
    ...
    try {
      ...
      setApprovalMessage(
        res.data.message || `Action ${action} processed successfully.`,
      );
      await fetchLoans();
    } catch (err) {
      setApprovalError(
  ...
  const handleRepaymentSubmit = async (e) => {
    ...
    try {
      ...
      setRepayMessage(
        res.data.message || "Repayment request submitted successfully.",
      );
      setRepayAmount("");
      setRepayLoanId("");
      fetchRepayments();
  ...
  const availableGuarantorList = useMemo(() => {
    return members.filter(
      (m) => String(m.id) !== String(selectedMember || user?.id),
    );
  ...
  const pendingCommitteeLoans = loans.filter(
    (l) =>
      ...
      (l.tier === "TIER_2" || l.loan_type === "TIER_2"),
  );
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Apply for loans, manage repayments, and track schedules.
        </p>
      </div>
        ...
        {isCommitteeMember && (
          ...
          >
            ...
            {pendingCommitteeLoans.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-xs bg-amber-100 text-amber-800 font-bold rounded-full">
                {pendingCommitteeLoans.length}
              </span>
            )}
          </button>
        )}
          ...
          <form onSubmit={handleApplyLoan} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={loanTier}
                  onChange={(e) => setLoanTier(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                >
                  <option value="TIER_1">
                    Tier 1 - Emergency (Max 300k RWF)
                  </option>
            ...
            {loanTier === "TIER_1" && (
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                <label className="block text-xs font-semibold text-slate-600 mb-2">
                  Select 2 Guarantors (Cannot be yourself)
                </label>
                <div className="grid grid-cols-2 gap-4">
                  <select
                    value={guarantor1}
            ...
            {loanTier === "TIER_2" && (
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                ...
                <input
                  ...
                  placeholder="Collateral Estimated Value (RWF)"
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mb-2"
                />
                <input
                  type="text"
                  ...
                  placeholder="Collateral Document Ref (UPI/Reg #)"
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
                  required
                />
              </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-coop-primary hover:bg-blue-900 text-white font-semibold rounded-lg text-sm disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
            ...
            <form onSubmit={handleRepaymentSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-2">
                  Select Active Loan & View Balance Owed{" "}
                ...
                >
                  ...
                  {myActiveLoans.map((l) => {
                    const principal = Number(
                      l.principal_amount || l.amount || 0,
                    );
                    const remainingPrincipal = Number(
                      l.remaining_principal ?? principal,
                    );
                    const profitRate = Number(l.profit_rate || 0.05);
                    const profitAmount = Number(
                      l.profit_amount ??
                        l.interest_amount ??
                        remainingPrincipal * profitRate,
                    );
                    const totalDue = Number(
                      l.total_due ??
                        remainingPrincipal +
                          profitAmount +
                          Number(l.accrued_penalty_balance || 0) +
                          Number(l.accrued_admin_fee || 0),
                    );

                    return (
                      <option key={l.id} value={l.id}>
                        Loan #{l.id} | Borrowed: {principal.toLocaleString()}{" "}
                        RWF | Total Due: {totalDue.toLocaleString()} RWF
                      </option>
                    );
                  })}
                </select>
                  ...
                  >
                    <div>
                      <div className="font-semibold text-slate-900 text-sm">
                        Loan #{r.loan_id} - {Number(r.amount).toLocaleString()}{" "}
                        RWF
                      </div>
                      <div className="text-xs text-slate-500 mt-0.5">
                        Member ID: {r.member_id} | Status:{" "}
                        <span
                          className={`font-medium ${r.status === "APPROVED" ? "text-emerald-600" : r.status === "REJECTED" ? "text-red-600" : "text-amber-600"}`}
                        >
                          {r.status}
                        </span>
                      </div>
                    ...
                    {isCommitteeMember && r.status === "PENDING" && (
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            handleRepaymentApproval(r.id, "APPROVE")
                          }
                        ...
                        <button
                          onClick={() =>
                            handleRepaymentApproval(r.id, "REJECT")
                          }
                          disabled={repaymentActionLoading[r.id]}
      ...
      {activeTab === "approvals" && isCommitteeMember && (
        <div className="p-6 bg-white rounded-xl border border-slate-200 shadow-sm space-y-6">
          <div>
            <h3 className="font-bold text-slate-800 text-lg">
              Pending Tier 2 Multi-Sig Queue
            </h3>
            <p className="text-xs text-slate-500">
              Tier 2 development loans require multi-signature approval from
              Chairperson, Treasurer, and Auditor.
            </p>
          </div>
            ...
            <div className="space-y-4">
              {pendingCommitteeLoans.map((loan) => {
                ...
                return (
                  <div
                    key={loan.id}
                    className="p-5 border border-slate-200 rounded-xl bg-slate-50/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
                  >
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3 pt-2">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${loan.approved_by_chairperson ? "bg-emerald-100 text-emerald-800 font-medium" : "bg-slate-200 text-slate-600"}`}
                        >
                          {loan.approved_by_chairperson ? (
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${loan.approved_by_treasurer ? "bg-emerald-100 text-emerald-800 font-medium" : "bg-slate-200 text-slate-600"}`}
                        >
                          {loan.approved_by_treasurer ? (
                            <Check className="w-3 h-3" />
                          ) : (
                            <Clock className="w-3 h-3" />
                          )}{" "}
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 ${loan.approved_by_auditor ? "bg-emerald-100 text-emerald-800 font-medium" : "bg-slate-200 text-slate-600"}`}
                        >
                          {loan.approved_by_auditor ? (
                            <Check className="w-3 h-3" />
                          ) : (
                            <Clock className="w-3 h-3" />
                          )}{" "}
                    ...
                    <div className="flex items-center gap-2 self-end md:self-center">
                      {userSigned ? (
                        ...
                        <button
                          onClick={() =>
                            handleCommitteeApproval(loan.id, "APPROVE")
                          }
                          disabled={isLoading}
                      ...
                      <button
                        onClick={() =>
                          handleCommitteeApproval(loan.id, "REJECT")
                        }
                        disabled={isLoading}
